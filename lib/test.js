...
function parseErrorLine(result, inputLineNo, outputLineNo, input) {
  ...
  var x = input.indexOf("^");
  if (result.cursorLineNo === outputLineNo && result.cursorX < x) {
    x++;
  }
  result.error = {
    name: match[1],
...
function parseTabStopsLine(result, inputLineNo, outputLineNo, input) {
  ...
  for (x=0; x<input.length; x++) {
    ...
    if (!tabStop.ch) {
      throw error(inputLineNo, 'tabStop at ' + tabStop.x + ' does not point to open paren');
    }
...
function parseCaretLine(result, inputLineNo, outputLineNo, input) {
  return (
    parseErrorLine(result, inputLineNo, outputLineNo, input) ||
    parseTabStopsLine(result, inputLineNo, outputLineNo, input)
  );
}
...
function parseOutput(text) {
  ...
  for (i=0; i<lines.length; i++) {
    ...
    if (parseCaretLine(result, i, j-1, input)) {
      continue;
    }
