Parinfer.parenMode = (function(){
  ...
  function reinsertDelims(state) {
    for (var i=0; i<state.removedDelims.length; i++) {
      appendDelimTrail(state);
    }
  }

  function postProcessLine(state) {
    indentMode.removeDelimTrail(state);
    reinsertDelims(state);
  }
