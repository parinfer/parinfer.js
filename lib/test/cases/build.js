...
function finalizeCase(testCase) {
  try {
    var inData = parinferTest.parseInput(testCase.in.fileText); // {text, options}
    inData.fileText = testCase.in.fileText;
    inData.fileLineNo = testCase.in.fileLineNo;
    testCase.in = inData;
  }
  catch (e) {
    console.log();
  ...
  try {
    testCase.out.result = parinferTest.parseOutput(testCase.out.fileText); // {text, ...}
  }
  catch (e) {
    console.log();
