...
function handlePressedEnter(result, block, fileLineNo, x) {
  if (result.currLabel === "out") {
    error(fileLineNo, "no ^ pressedEnter annotation allowed in output block.");
  }
  if (block.cursor === null || block.cursor.cursorX !== x) {
    error(fileLineNo, "the ^ pressedEnter annotation does not point to a cursor.");
  }

  block.cursor.pressedEnter = true;
  return true;
}
...
function handleCaretLine(result, block, fileLineNo, line) {
  ...
  var pressedEnterMatch = /^\s*\^\s*pressedEnter\s*$/.exec(line);
  ...
  else if (pressedEnterMatch) {
    return handlePressedEnter(result, block, fileLineNo, x);
  }
  else if (errorMatch) {
