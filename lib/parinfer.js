...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    repeatChar: false,         // [boolean] - should we repeat the processing of the current character?
...
function onUnmatchedCloseParen(result) {
  ...
  if (result.cursorLine === result.lineNo &&
      ...
      peekn(result.parenStack, 2).ch === PARENS[result.ch]) {

    result.ch = PARENS[peek(result.parenStack).ch];
    onMatchedCloseParen(result);
    result.repeatChar = true;
    return;
  }
  result.ch = "";
}
