...
function getInitialResult() {
  ...
  var result = {
    ...
    topLevelLineNums: [],   // [array of integers] - line number of each top-level open-paren
...
function pushOpen(result) {
  ...
  if (isEscaping(stack)) {
    ...
    if (result.readOnly && stack.length === 0) {
      result.topLevelLineNums.push(result.lineNo);
    }
    stack.push({
      x: result.x,
      ch: result.ch,
