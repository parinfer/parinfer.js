...
function getParentOpenerIndex(result, indentX) {
  ...
  for (i=0; i<result.parenStack.length; i++) {
    ...
    /* TODO: REMOVE INSPECTION */
    console.log('currOutside', currOutside);
    console.log('prevOutside', prevOutside);
    ...
    if (currOutside) {
      ...
      if (opener.indentDelta === 0) {
        ...
        // If the opener and its parent opener were not pushed by same amount,
...
function onIndent(result) {
  ...
  if (result.mode === INDENT_MODE) {
    ...
    /* TODO: REMOVE INSPECTION */
    if (result.lineNo === 2) {
      console.log("parenStack", JSON.stringify(result.parenStack, null, 2));
      console.log("indentDelta", JSON.stringify(result.indentDelta, null, 2));
    }
