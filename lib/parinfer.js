...
function test_checkDiffLine(inputLineNo, input, diff) {
  // 'diff' param is a running diff state
  ...
  if (!diff.code) {
    test_error(inputLineNo, looseDesc + " diff lines must be directly below a code line");
  }
  if (diff.prevCode) {
    if (diff.prevCodeLineNo !== diff.codeLineNo &&  // happens when lines are merged
        diff.prevCodeLineNo+1 !== diff.codeLineNo) {
      test_error(inputLineNo, looseDesc + " multiple diff lines are currently not supported unless they are adjacent");
    }
    if (!diff.prevMerge) {
      test_error(inputLineNo, "the newline char of the previous line must have a diff char to connect it to this diff line");
    }
    if (diff.prevCode[diff.prevCode.length-1] === '+' && diff.code[0] === '-') {
...
function test_parseDiffLine(options, inputLineNo, input, diff) {
  ...
  var newlineOld = (diff.code.length === xEnd && input.charAt(xEnd) === '-');
  var newlineNew = (diff.code.length === xEnd && input.charAt(xEnd) === '+');
  var oldText = diff.code.substring(oldX, oldX+oldLen) + (newlineOld ? '\n' : '');
  var newText = diff.code.substring(newX, newX+newLen) + (newlineNew ? '\n' : '');
...
function test_handlePostDiffLine(options, inputLineNo, outputLineNo, outputLines, output, diff) {
  ...
  if (diff.merge) {
    if (options.cursorLine === j) {
      ...
      options.cursorX += outputLines[j-1].length;
    }
    j--;
    outputLines[j] += output;
    diff.prevMerge = true;
    diff.merge = false;
  } else {
    outputLines.push(output);
    diff.prevMerge = false;
  }
  // FIXME: diff actually need to remember the original input line which it annotates, not the possibly merged output line.
  diff.code = outputLines[j];
  diff.codeLineNo = j;
}
