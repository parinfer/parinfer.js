...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    firstUnmatchedCloseParen: {
      x: SENTINEL_NULL,        // [integer] - x position of the first unmatched close paren of a line (if any)
      inputX: SENTINEL_NULL    // [integer] - in input coords
    },
...
function error(result, name) {
  ...
  if (name === ERROR_UNMATCHED_CLOSE_PAREN) {
    if (result.firstUnmatchedCloseParen.inputX !== SENTINEL_NULL) {
      e.x = result.firstUnmatchedCloseParen.inputX;
    }
...
function initLine(result, line) {
  ...
  result.firstUnmatchedCloseParen.x = SENTINEL_NULL;
  result.firstUnmatchedCloseParen.inputX = SENTINEL_NULL;
}
...
function onUnmatchedCloseParen(result) {
  ...
  if (result.firstUnmatchedCloseParen.x === SENTINEL_NULL) {
    result.firstUnmatchedCloseParen.x = result.x;
    result.firstUnmatchedCloseParen.inputX = result.inputX;
    result.parenTrail.endX = result.x + 1;
  }
}
...
function processLine(result, lineNo) {
  ...
  var unmatchedX = result.firstUnmatchedCloseParen.x;
  if (unmatchedX !== SENTINEL_NULL && unmatchedX < result.parenTrail.startX) {
    var e = error(result, ERROR_UNMATCHED_CLOSE_PAREN);
    e.x = result.firstUnmatchedCloseParen.inputX;
    throw e;
  }
