...
/// These long comments below are for the purpose of assisting investigation
/// towards what I'm calling the "multi-change bug" for Smart Mode. In summary,
/// throwing multiple changes at Smart Mode can sometimes produce bad code
/// because the way we determine how to shift expressions is currently suited
/// only for simpler changes.
/// TODO: move comments to doc/code.md
...
function getParentOpenerIndex(result, indentX) {
  ...
  for (i=0; i<result.parenStack.length; i++) {
    ...
    // "OUTSIDE" means that an opener can be considered a PARENT of the
    // following line. That is, the open-paren is _outside_ the indentation point.
    // In other words, the open-paren is to the left of the indentation point.
    //
    ...
    // an open-paren as a "true intended" parent of the current line.
    //
    ...
    // TODO: Investigate why dedentation is being ignored with Math.max
    ...
    if (currOutside) {
      // Here, we know an open-paren was JUST pushed outside (implied by prevOutside being false)

      if (opener.indentDelta === 0) {
        // If the opener didn't move, then the indentation point moved to the right.
        // Nothing special has to happen.  The opener is truly the correct parent.
        break;
      } else {
        // If the opener and its parent opener was not pushed by same amount,
        // new child line(s) will be adopted.
        // Clear `indentDelta` since it is reserved for previous child lines only.

        // TODO: clarify why this works.  I believe this is to prevent redundant
        //  indentation correction, though I'm not certain of this anymore.
        //  Will investigate.
