...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    isNewlineMode: false,      // [boolean] - Newline Mode is just a flag for Paren Mode
    insertedLineNums: [],         // [array of line numbers] -
...
function insertLineAt(result, lineNo, line) {
  result.insertedLineNums.push(lineNo);
...
function splitLineForStability(result) {
  if (isCloseParen(result.ch)) {
    ...
    if (i !== -1 && line[i] !== ';') {
      // Here, we know this line has tokens after the close-parens, so we split
      // this line to prevent the tokens from being auto-slurped by Indent Mode.
      var lineA = line.substring(0, i);
      var lineB = line.substring(i);

      result.lines[result.lineNo] = lineA;
...
function onIndent(result) {
  ...
  if (result.mode === INDENT_MODE) {
    ...
    if (result.isNewlineMode && result.cursorLine === result.lineNo) {
...
function getNewLineNo(result, origLineNo) {
  var newLineNo = origLineNo;
  var i;
  for (i = 0; i < result.insertedLineNums.length; i++) {
    if (result.insertedLineNums[i] < newLineNo) { break; }
    newLineNo++;
  }
  return newLineNo;
}
...
function getChangedLines(result) {
  ...
  var i;
  for (i = 0; i < result.origLines.length; i++) {
    var newI = getNewLineNo(result, i);
    if (result.lines[newI] !== result.origLines[i]) {
      changedLines.push({
        lineNo: i,
        line: result.lines[newI]
      });
    }
  }
  return changedLines;
...
function getInsertedLines(result) {
  var insertedLines = [];
  var i;
  for (i = 0; i < result.insertedLineNums.length; i++) {
    var lineNo = result.insertedLineNums[i];
    insertedLines.push({
      lineNo: lineNo,
...
function publicResult(result) {
  ...
  return {
    ...
    insertedLines: getInsertedLines(result)
  }
}
