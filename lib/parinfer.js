...
var ERROR_QUOTE_DANGER = "QUOTE_DANGER";
//var ERROR_EOL_BACKSLASH = "EOL_BACKSLASH";
//var ERROR_UNCLOSED_QUOTE = "UNCLOSED_QUOTE";
//var ERROR_UNCLOSED_PAREN = "UNCLOSED_PAREN";
...
function onChar(result) {
  ...
  else if (isOpenParen(result.ch))  { onOpenParen(result); }
  else if (isCloseParen(result.ch)) { onCloseParen(result); }
  else {
    switch (result.ch) {
      case DOUBLE_QUOTE:            onQuote(result); break;
      case SEMICOLON:               onSemicolon(result); break;
      case BACKSLASH:               onBackslash(result); break;
      case TAB:                     onTab(result); break;
      case NEWLINE:                 onNewline(result); break;
    }
  }
...
function postProcessNewParenTrail(result) {
...
function onIndentationPoint(result) {
  ...
  if (result.quoteDanger) {
    throw error(ERROR_QUOTE_DANGER, null);
  }
...
function processLine(result, line) {
  ...
  if (result.lineNo === result.parenTrail.lineNo) {
    postProcessNewParenTrail(result);
  }
}

function finalizeResult(result) {
  result.success = !result.isInStr && !result.quoteDanger;
  var unclosedParens = (result.parenStack.length !== 0);

  if (result.mode === INDENT_MODE) {
    if (unclosedParens && result.success) {
...
function processText(text, options, mode) {
  ...
  try {
    ...
    // TODO: add error messages to result
    result.success = false;
  }
...
function modeFunction(text, options, mode) {
  var lineEnding = getLineEnding(text);
  var result = processText(text, options, mode);
...
function indentMode(text, options) {
  return modeFunction(text, options, INDENT_MODE);
}

function parenMode(text, options) {
  return modeFunction(text, options, PAREN_MODE);
}
