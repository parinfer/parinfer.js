...
function parseOptions(options) {
  ...
  return {
    ...
    change: parseChange(options.change),
...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    //   lineNo: n,
    //   x: n
    //   oldText: null,        // the text replaced by the edit
    //   newText: null         // the text that replaces oldText
    // },
  ...
  if (options) {
    ...
    if (isObject(options.change))              { result.change             = options.change; }
...
function handleChangeDelta(result) {
  ...
  var hasChangeDelta = (
    result.lineNo === result.change.newEndLineNo &&
    result.x === result.change.newEndX
  );

  if (hasChangeDelta) {
    result.indentDelta += (result.change.newEndX - result.change.oldEndX);
  }
}
...
function test_parseDiffLine(options, inputLineNo, input, diff) {
  ...
  if (!options.change) {
    options.change = {lineNo: diff.codeLineNo, x: x, oldText: '', newText: ''};
  }
...
function testInput(text) {
  ...
  console.log('options:', parseOptions(parsed.options));
}
