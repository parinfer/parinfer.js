...
function peek(stack, i) {
  ...
  // return null if the index is out of range or we can't find the next-to-last
...
function isEscaping(stack) {
  return getPrevCh(stack, null) === BACKSLASH;
}
...
function isValidCloser(stack, ch) {
  return getPrevCh(stack, null) === PARENS[ch];
}
...
function minIndent(x, result) {
  var opener = peek(result.stack, null);
  if (opener !== null) {
    var startX = opener.x;
