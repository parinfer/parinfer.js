...
function isValidCloser(parenStack, ch) {
...
function pushCloseParen(result) {
  if (result.isInCode) {
    if (isValidCloser(result.parenStack, result.ch)) {
...
function removeParenTrail(result) {
  ...
  var ignoreCount = openers.length - removeCount;
  while (ignoreCount !== openers.length) {
...
function processIndent_paren(result) {
  ...
  var atValidCloser = (
    checkIndent &&
    closeParen &&
    isValidCloser(result.parenStack, ch)
  );
