...
function test_parseDiffLine(options, inputLineNo, prevOutputLineNo, input, prevOutput) {
  ...
  if (options.cursorLine === prevOutputLineNo) {
  ...
  var oldText = prevOutput.substring(oldX, oldX+oldLen);
  var newText = prevOutput.substring(newX, newX+newLen);

  var output = replaceWithinString(prevOutput, oldX, oldX+oldLen, '');
...
function test_parse(text) {
  ...
  for (i=0; i<inputLines.length; i++) {
    ...
    var diff = test_parseDiffLine(options, i, j-1, input, prevOutput);
    if (diff) {
