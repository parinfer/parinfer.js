...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    cursorDx: UINT_NULL,       // [integer] - amount that the cursor moved horizontally if something was inserted or deleted
...
function error(result, name) {
  ...
  var opener = peek(result.parenStack, 1);
...
function isValidCloseParen(parenStack, ch) {
  ...
  return peek(parenStack, 1).ch === MATCH_PAREN[ch];
}
...
function correctParenTrail(result, indentX) {
  ...
  while (result.parenStack.length > 0) {
    ...
    if (currOutside) {
      ...
      var nextOpener = peek(result.parenStack, 2);
