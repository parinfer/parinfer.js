...
function initialParenTrail() {
  return {
    ...
    clamped: {
      ...
      openers: null          // openers that were cut out after paren trail was clamped
    }
  };
}
...
function getInitialResult(text, options, mode, smart) {
  ...
  var result = {
    ...
    smart: smart === 'smart',  // [boolean] - smart mode attempts special user-friendly behavior
...
function clampParenTrailToCursor(result) {
  ...
  var shouldCheckPrev = (result.smart && !result.changes);
  if (shouldCheckPrev) {
...
function rememberParenTrail(result) {
  ...
  var openers = trail.openers;
  if (isClamped) {
    openers = trail.clamped.openers.concat(openers);
  }
  if (openers.length > 0) {
    var shortTrail = {
      lineNo: trail.lineNo,
