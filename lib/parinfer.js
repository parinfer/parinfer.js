...
function getInitialResult(text, options, mode) {
  ...
  var result = {
    ...
    cameFromIndentMode: false, // [boolean] - special case when mode=PAREN_MODE after exiting from INDENT_MODE
...
function onLeadingCloseParen(result) {
  ...
  if (result.mode === PAREN_MODE) {
    ...
    else if (!result.cameFromIndentMode) {
      // only safe to ignore leading unmatched close-parens if we came from Indent Mode
      throw error(result, ERROR_UNMATCHED_CLOSE_PAREN, result.inputLineNo, result.inputX);
    }
  }
}
...
function processText(text, options, mode) {
  ...
  try {
    ...
    if (e.indentModeLeadingCloseParen) {
      options.cameFromIndentMode = true;
...
function test_errorLine(error) {
  return repeatString(" ", error.x) + "^ " + error.name;
}
...
function test_format(result, printOptions) {
  ...
  if (result.error) {
    lines.splice(result.error.lineNo+1, 0, test_errorLine(result.error));
  } else if (result.tabStops.length > 0 && printOptions && printOptions.tabStops) {
...
function testIndentMode(text, printOptions) {
  ...
  console.log(test_format(result, printOptions));
}
