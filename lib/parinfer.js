...
function test_parseDiffLine(options, inputLineNo, outputLineNo, input, prevOutput) {
  var looseMatch = input.match(/^[-+\s]+$/);
  var looseDesc = "lines with only -/+ chars are considered diff lines.";
  if (!looseMatch) {
    return;
  }
  ...
  if (!match) {
    test_error(inputLineNo, looseDesc + " diff chars must be adjacent with '-'s before '+'s.");
  }

  var oldX = input.indexOf('-');
  var newX = input.indexOf('+');
  var oldLen = match[2].length;
  var newLen = match[3].length;
  var len = oldLen + newLen;
  var x = match[1].length;
  var xEnd = x+len-1;

  if (options.cursorLine === outputLineNo-1) {
    if (options.cursorX < x) {
      x--; oldX--; newX--;
    } else if (x <= options.cursorX && options.cursorX < x+len) {
  ...
  if (!prevOutput) {
    test_error(inputLineNo, "diff line must be underneath another line");
  }
  var oldText = prevOutput.substring(oldX, oldLen);
  var newText = prevOutput.substring(newX, newLen);

  var output = replaceWithinString(prevOutput, x, len, '');

  if (xEnd > prevOutput.length) {
    test_error(inputLineNo, "diff line can only extend one character after previous line (last diff char annotates the 'newline' char)");
  }
  var newlineAdded =   (prevOutput.length === xEnd && input.charAt(xEnd) === '+');
  var newlineRemoved = (prevOutput.length === xEnd && input.charAt(xEnd) === '-');

  // TODO: merge with previous options.change
  options.change = {
    position: {
      lineNo: outputLineNo,
      x: x
    },
    oldText: oldText + (newlineRemoved ? '\n' : ''),
    newText: newText + (newlineAdded ? '\n' : '')
  };

  return {
    output: output,
    newlineRemoved: newlineRemoved
  };
}
...
function test_parseCursorFromLine(options, inputLineNo, input) {
  ...
  if (cursorX !== -1) {
    ...
    options.cursorLine = inputLineNo;
  }
  return input;
}
...
function test_parse(text) {
  ...
  var prevOutput;
  var outputMergePrev = false;

  for (i=0; i<inputLines.length; i++) {
    input = inputLines[i];
    j = outputLines.length;
    prevOutput = outputLines[j-1];

    if (test_parseCaretLine(options, i, input)) {
      continue;
    }

    var diff = test_parseDiffLine(options, i, j, input, prevOutput);
    if (diff) {
      outputLines[j-1] = diff.output;
      outputMergePrev = diff.newlineRemoved;
      continue;
    }

    output = test_parseCursorFromLine(options, i, input);
    if (outputMergePrev) {
      outputMergePrev = false;
      outputLines[j-1] = prevOutput + output;
      if (options.cursorLine === j) {
        options.cursorLine = j-1;
        options.cursorX += prevOutput.length;
      }
    } else {
